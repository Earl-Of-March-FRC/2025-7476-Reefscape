...
@REM Pull from GitHub (if possible)
@echo Attempting to pull from GitHub (if possible)
@git pull
...
@REM Install requirements and run the module using -m
@echo Installing PIP requirements and running the module
@ssh %ADDRESS% "cd %REMOTE_DIR%/%PY_DIRECTORY% && pip install -r requirements.txt && python3 -m python.algae.algaeClassification" || goto end
